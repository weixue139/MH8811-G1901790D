#Program H1 take C3 program and add variance calculation and output all statistics
...
#define function to calculate sample variance
def var():
    n=0
    for i in L:
        n=n+1
    
    #call my_average function to define sample mean    
    avg=my_average()
    
    #use loop to calculate sum of the square of the difference between each input and the mean
    square=0
    sumsquare=0
    for x in L:
        square = (x-avg)**2
    ...
    #calculate variance and round to 3 decimal places, then return var
    var=round(sumsquare/(n-1),3)
    return var
   
#output all the statistics from C3 and sample variance
